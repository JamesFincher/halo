#!/usr/bin/env python3
"""D174: halo ratchet --json includes scores_count trajectories_count scores_trajectories_match."""
...
class TestRatchetScoresTrajectories(unittest.TestCase):
    ...
    def test_ratchet_json_has_counts_and_match_when_present(self) -> None:
    ...
    def test_ratchet_json_match_false_when_counts_diverge(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._git_init(repo)
            halo = self._scaffold_halo(repo)
    ...
    def test_ratchet_json_zeros_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._git_init(repo)
