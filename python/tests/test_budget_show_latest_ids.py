#!/usr/bin/env python3
"""D142: halo budget show JSON includes latest_score_id and latest_trajectory_id."""
...
class TestBudgetShowLatestIds(unittest.TestCase):
    ...
    def test_show_has_latest_ids_when_present(self) -> None:
    ...
    def test_show_latest_ids_null_when_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            (halo / "scores").mkdir()
    ...
    def test_show_latest_ids_null_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._scaffold(repo)
            data = self._show(repo)
