#!/usr/bin/env python3
"""D137: halo go --plan JSON includes scores_count trajectories_count scores_trajectories_match."""
...
class TestGoPlanScoresTrajectories(unittest.TestCase):
    ...
    def test_plan_has_counts_and_match_true_when_equal(self) -> None:
    ...
    def test_plan_match_false_when_diverge(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            scores = halo / "scores"
    ...
    def test_plan_zero_counts_match_true(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._scaffold(repo)
            data = self._plan(repo)
