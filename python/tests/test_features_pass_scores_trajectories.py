#!/usr/bin/env python3
"""D147: halo features pass JSON includes scores_count trajectories_count scores_trajectories_match."""
...
class TestFeaturesPassScoresTrajectories(unittest.TestCase):
    ...
    def test_pass_json_has_counts_and_match_after_mark(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # on_feature_pass adds one score + one trajectory after mark
            self.assertEqual(data["scores_count"], 3)
            self.assertEqual(data["trajectories_count"], 3)
            self.assertTrue(data["scores_trajectories_match"])
            # feature-list shape still present
            self.assertIn("features", data)
            self.assertEqual(data["version"], 1)

    def test_pass_json_match_false_when_diverge_after_mark(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            scores = halo / "scores"
