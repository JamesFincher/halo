#!/usr/bin/env python3
"""D173: halo progress add JSON includes latest_score_id and latest_trajectory_id."""
...
class TestProgressAddLatestIds(unittest.TestCase):
    ...
    def test_unit_add_json_has_latest_ids_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # D172 fields remain present alongside latest ids
            self.assertIn("scores_count", data)
            self.assertEqual(data["scores_count"], 3)

    def test_unit_add_json_latest_ids_null_when_dirs_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            (halo / "scores").mkdir()
    ...
    def test_unit_add_json_latest_ids_null_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._scaffold(repo)
            data = self._add(repo, "unit")
