#!/usr/bin/env python3
"""D141: halo budget show JSON includes scores_count trajectories_count scores_trajectories_match."""
...
class TestBudgetShowScoresTrajectories(unittest.TestCase):
    def _show(self, repo: Path) -> dict:
        r = subprocess.run(
            [
                sys.executable,
    ...
    def test_show_has_counts_and_match_true_when_equal(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # D097 fields still present
            self.assertIn("spend", data)
            self.assertIn("max_iterations", data)

    def test_show_match_false_when_diverge(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            scores = halo / "scores"
    ...
    def test_show_zero_counts_match_true(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._scaffold(repo)
            data = self._show(repo)
