#!/usr/bin/env python3
"""D170: halo evidence check JSON includes scores_count trajectories_count scores_trajectories_match."""
...
class TestEvidenceCheckScoresTrajectories(unittest.TestCase):
    ...
    def test_check_json_has_counts_and_match_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            evidence = halo / "evidence" / "D170-green.json"
    ...
    def test_check_json_match_false_when_counts_diverge(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            scores = halo / "scores"
    ...
    def test_check_json_zeros_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            evidence = halo / "evidence" / "D170-green.json"
