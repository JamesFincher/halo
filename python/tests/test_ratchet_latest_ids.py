#!/usr/bin/env python3
"""D175: halo ratchet --json includes latest_score_id and latest_trajectory_id."""
...
class TestRatchetLatestIds(unittest.TestCase):
    def _ratchet_json(self, repo: Path) -> dict:
        r = subprocess.run(
            [
                sys.executable,
                str(ROOT / "halo_ratchet.py"),
                "--repo",
                str(repo),
                "--json",
            ],
    ...
    def test_ratchet_json_has_latest_ids_when_present(self) -> None:
    ...
    def test_ratchet_json_latest_ids_null_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._git_init(repo)
    ...
    def test_ratchet_json_latest_ids_null_when_dirs_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._git_init(repo)
            halo = self._scaffold_halo(repo)
