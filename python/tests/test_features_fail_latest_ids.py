#!/usr/bin/env python3
"""D149: halo features fail JSON includes latest_score_id and latest_trajectory_id."""
...
class TestFeaturesFailLatestIds(unittest.TestCase):
    ...
    def test_fail_json_has_latest_ids_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # fail does not call on_feature_pass — ids stay pre-existing max
    ...
    def test_fail_json_latest_ids_null_when_dirs_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            halo = self._scaffold(repo)
            (halo / "scores").mkdir()
    ...
    def test_fail_json_latest_ids_null_when_dirs_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            self._scaffold(repo)
            data = self._fail(repo)
