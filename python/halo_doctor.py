...
CORE_SPEC_FILES = ("PRD.md", "STORIES.md", "STACK.md")

EXPANDED_SPEC_FILES = [
    "DATA-MODEL.md", "DESIGN.md", "ARCHITECTURE.md", "INTEGRATIONS.md", "MILESTONES.md",
    "READINESS.md", "API.md", "USER-FLOWS.md", "ARCHITECTURE-DECISIONS.md", "SEQUENCE.md",
    "STATE.md", "SECURITY.md", "TEST-PLAN.md", "FRONTEND.md", "BACKEND.md", "MOBILE.md",
    "DEPLOYMENT.md", "RUNBOOK.md", "METRICS.md", "PROMPTS.md", "GLOSSARY.md", "RISKS.md",
    "PERSONAS.md", "SEED.md", "CHANGELOG.md", "CONTRIBUTING.md",
]
...
def _compounding_score_culture_issues(repo: Path) -> list[dict[str, Any]]:
    ...
    # D161 / D114: empty trajectories (list_trajectories culture — GT-*.json only)
    if tn == 0:
        issues.append(
            {
                "level": "warn",
                ...
                "item": (
                    "compounding self-instance with empty .halo/trajectories "
                    "(list_trajectories count=0) — land a trajectory GT-###"
                ),
            }
        )
...
def check_product(repo: Path) -> list[dict[str, Any]]:
    ...
    if state.get("spec_status") == "locked":
        spec = repo / ".halo" / "spec"
        for f in CORE_SPEC_FILES:
            if not (spec / f).exists():
                issues.append({"level": "error", "code": "locked_missing_spec", "item": f})
        # Expanded spec pack is only required for states generated by the new verbose writer.
        # Older locked states warn so existing dogfood instances don't break on upgrade.
        spec_pack_version = int(state.get("spec_pack_version") or 0)
        for f in EXPANDED_SPEC_FILES:
            if not (spec / f).exists():
                if spec_pack_version >= 2:
                    issues.append({"level": "error", "code": "locked_missing_spec", "item": f})
                else:
                    issues.append({"level": "warn", "code": "missing_expanded_spec", "item": f})
